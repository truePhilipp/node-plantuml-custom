...
var PlantumlSyntaxError = require('./plantuml-syntax-error')
...
function generate (file, options) {
  ...
  var gen
  var syntaxErrorOccured = false

  function generateCallback (error, data) {
    if (error instanceof PlantumlSyntaxError) {
      syntaxErrorOccured = true
    }
  }

  if (file) {
    gen = plantuml.generate(file, options, generateCallback)
  } else if (options.text) {
    options.text = '@startuml' + os.EOL + options.text + os.EOL + '@enduml'
    gen = plantuml.generate(options.text, options, generateCallback)
  } else {
    gen = plantuml.generate(options, generateCallback)
